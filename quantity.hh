...
namespace Optima::Control {
	...
	namespace Prefix {
		...
		struct Pow { using type = typename Prefix::Multiply<Ratio, typename Pow<typename Ratio::type, D - 1>::type>::type; };

		template <typename Ratio>
		struct Pow<Ratio, 1> { using type = typename Ratio::type; };
	...
	namespace SI {
		...
		>
			...
			template <T os, T om, T okg, T oA, T oK, T oMol, T ocd>
			constexpr bool operator==(UnitSI<os, om, okg, oA, oK, oMol, ocd> const&) {
...
#pragma region BASIC_SI
		...
		constexpr inline Time _second;
		constexpr inline Length _meter;
		constexpr inline Mass _kg;
		constexpr inline Current _amp;
		constexpr inline ThermoTemperature _kelvin;
		constexpr inline SubstanceAmount _mole;
		constexpr inline LuminousIntensity _candela;
#pragma endregion

#pragma region DERIVED_QUANTITIES
		using Velocity = decltype(_meter / _second);
		using Acceleration = decltype(Velocity{} / _second);
		using Force = decltype(_kg* Acceleration{});
		using Power = decltype(_kg* _meter* _meter / (_second * _second * _second));
		using Voltage = decltype(Power{} / _amp);
		using Resistance = decltype(Voltage{} / _amp);
		using Area = decltype(_meter* _meter);
		using Volume = decltype(Area{} *_meter);
#pragma endregion

#pragma region VALIDATOR
		template<typename Q2>	constexpr inline bool IsQuantityValid = false;
		template<>	constexpr inline bool IsQuantityValid<SI::Scalar> = true;
		template<>	constexpr inline bool IsQuantityValid<SI::Time> = true;
		template<>	constexpr inline bool IsQuantityValid<SI::Length> = true;
		template<>	constexpr inline bool IsQuantityValid<SI::Mass> = true;
		template<>	constexpr inline bool IsQuantityValid<SI::Current> = true;
		template<>	constexpr inline bool IsQuantityValid<SI::ThermoTemperature> = true;
		template<>	constexpr inline bool IsQuantityValid<SI::SubstanceAmount> = true;
		template<>	constexpr inline bool IsQuantityValid<SI::LuminousIntensity> = true;
		template<>	constexpr inline bool IsQuantityValid<SI::Velocity> = true;
		template<>	constexpr inline bool IsQuantityValid<SI::Acceleration> = true;
		template<>	constexpr inline bool IsQuantityValid<SI::Force> = true;
		template<>	constexpr inline bool IsQuantityValid<SI::Power> = true;
		template<>	constexpr inline bool IsQuantityValid<SI::Voltage> = true;
		template<>	constexpr inline bool IsQuantityValid<SI::Resistance> = true;
		template<>	constexpr inline bool IsQuantityValid<SI::Area> = true;
		template<>	constexpr inline bool IsQuantityValid<SI::Volume> = true;
#pragma endregion

#pragma region UTILITY
		template<typename Q>
		constexpr inline auto quantityType = "scalar";
		template<>  constexpr inline auto quantityType<SI::Time> = "time";
		template<>  constexpr inline auto quantityType<SI::Length> = "length";
		template<>  constexpr inline auto quantityType<SI::Mass> = "mass";
		template<>	constexpr inline auto quantityType<SI::Current> = "current";
		template<>	constexpr inline auto quantityType<SI::ThermoTemperature> = "thermodynamic_temperature";
		template<>	constexpr inline auto quantityType<SI::SubstanceAmount> = "amount_of_substance";
		template<>	constexpr inline auto quantityType<SI::LuminousIntensity> = "luminance";
		template<>	constexpr inline auto quantityType<SI::Velocity> = "velocity";
		template<>	constexpr inline auto quantityType<SI::Acceleration> = "acceleration";
		template<>	constexpr inline auto quantityType<SI::Force> = "force";
		template<>	constexpr inline auto quantityType<SI::Power> = "power";
		template<>	constexpr inline auto quantityType<SI::Voltage> = "voltage";
		template<>	constexpr inline auto quantityType<SI::Resistance> = "resistance";
		template<>	constexpr inline auto quantityType<SI::Area> = "area";
		template<>	constexpr inline auto quantityType<SI::Volume> = "volume";

		template<typename Q>
		constexpr auto getQuantityType() noexcept { return quantityType<Q>; }
#pragma endregion

	}  // namespace SI

	template <typename T, typename Q, typename P, std::size_t D = 1>
	...
	public:
		...
		static_assert(D > 0 && D < 5, "Quantity dimension is out of our support range");

		Quantity() = default;
		explicit constexpr Quantity(T value) noexcept : value{ value } {}
		explicit constexpr Quantity(T value, Q) noexcept : value{ value } {}
		explicit constexpr Quantity(T value, Q, P) noexcept : value{ value } {}
		explicit constexpr Quantity(T value, Q, P, std::size_t) noexcept : value{ value } {}
		...
		constexpr auto operator+(Quantity<T2, Q2, P2, D2> const& other) const noexcept {
			using commonPrefix = std::common_type_t<Rep, P2>;
			return Quantity<T2, Q2, commonPrefix, D2>{
		...
		template <typename T2, typename Q2, typename P2, std::size_t D2>
		constexpr auto operator*(Quantity<T2, Q2, P2, D2> const& other) const noexcept {
			static_assert(std::is_same_v<T, T2>, "Type must be same");

			auto resUnit = GetUnit() * other.GetUnit();
			using resQuantityType = decltype(resUnit);
			constexpr auto resDim = QuantityDimension<resQuantityType, D2>::dim;

			if constexpr (std::is_same_v<Q, Q2>) {
				using commonPrefix = std::common_type_t<Rep, P2>;
				return Quantity<T, resQuantityType, commonPrefix, resDim>{
					convertToDst<commonPrefix, Rep>()*
						other.template convertToDst<commonPrefix, P2>()};
			}
			else {
				if constexpr (resDim == 1) {
					using resRep = typename Prefix::Multiply<P, P2>::type;
					return Quantity<T, resQuantityType, resRep, resDim>{value* other.value};
				}
				else {
					// TODO: handle higher dimension for conversion without normalization
					return Quantity<T, resQuantityType, Prefix::None, resDim>{
						convertToDst<Prefix::None, Rep>()*
							other.template convertToDst<Prefix::None, P2>()};
				}
			}
		}
		...
		constexpr auto operator/(Quantity<T2, Q2, P2, D2> const& other) const noexcept {
			...
			if constexpr (std::is_same_v<Q, Q2>) {
				using resRep = typename Prefix::Divide<Rep, P2>::type;
				return Quantity<T, resQuantityType, resRep, resDim>{value / other.value};
			}
			else {
				using p1 = typename Prefix::Pow<P, D>::type;
				using p2 = typename Prefix::Pow<P2, D2>::type;
				using resRep = typename Prefix::Divide<p1, p2>::type;
				return Quantity<T, resQuantityType, resRep, resDim>{value / other.value};
			}
		}
		...
		constexpr auto getQuantityType() const noexcept {
			return SI::getQuantityType<Q>();
		}

		template <typename P2>
		constexpr auto cast() const noexcept {
			return Quantity<T, Q, P2, D>(convertToDst<P2, Rep>());
		}

		template <typename T2, typename Q2, typename P2, std::size_t D2>
		constexpr bool operator==(Quantity<T2, Q2, P2, D2> const& other) const noexcept {
			using comRep = std::common_type_t<Rep, P2>;
			return (std::is_same_v<T, T2>) && (std::is_same_v<Q, Q2>) && (D == D2) &&
				(convertToDst<comRep, Rep>() == other.template convertToDst<comRep, P2>());
		}
	...
	private:
		...
		constexpr T convertToDst() const noexcept {
			using Ratio = typename Prefix::Pow<std::ratio_divide<RepSrc, RepDst>, D>::type;
		...
		template <typename Q2, std::size_t D2>
		struct QuantityDimension {
			static constexpr std::size_t dim = D2;
		};
		template <std::size_t D2>
		struct QuantityDimension<SI::Area, D2> {
			static constexpr std::size_t dim = 2;
		};
		template <std::size_t D2>
		struct QuantityDimension<SI::Volume, D2> {
			static constexpr std::size_t dim = 3;
		};

		template <typename Q2>
		constexpr std::size_t getCorrectDim() const noexcept {
			if (std::is_same_v<Q2, SI::Area>) {
				return 2;
			}
			else if (std::is_same_v<Q2, SI::Volume>) {
				return 3;
			}
			return 1;
		}
...
#pragma region QUANTITY_AREA
	template <typename T = double>
	using KiloMeterSqr = Quantity<T, SI::Area, Prefix::Kilo, 2>;

	template <typename T = double>
	using MeterSqr = Quantity<T, SI::Area, Prefix::None, 2>;

	template <typename T = double>
	using MilliMeterSqr = Quantity<T, SI::Area, Prefix::Milli, 2>;
#pragma endregion

#pragma region QUANTITY_VOLUME
	template <typename T = double>
	using KiloMeterCub = Quantity<T, SI::Volume, Prefix::Kilo, 3>;

	template <typename T = double>
	using MeterCub = Quantity<T, SI::Volume, Prefix::None, 3>;

	template <typename T = double>
	using MilliMeterCub = Quantity<T, SI::Volume, Prefix::Milli, 3>;
#pragma endregion
...
namespace std {
	...
	{
		using type = typename common_type_t<
			chrono::duration<int, ratio<N0, D0>>,
			chrono::duration<int, ratio<N1, D1>>
		>::period;
	};
}  // namespace std
#pragma endregion
